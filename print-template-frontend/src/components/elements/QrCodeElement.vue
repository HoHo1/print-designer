<template>
  <div
    class="element-wrapper qrcode-element"
  ...
  >
    <div class="element-content" :style="contentStyle">
      <canvas ref="canvasRef" :width="Math.min(element.width, element.height)" :height="Math.min(element.width, element.height)"></canvas>
    </div>
...
function renderQrCode() {
  ...
  try {
    const size = Math.min(props.element.width, props.element.height)
    QRCode.toCanvas(canvasRef.value, props.element.options.content || 'https://example.com', {
      width: size,
      margin: 2,
      color: {
        dark: props.element.options.color || '#000000',
        light: '#ffffff'
      }
    })
  } catch (e) {
...
watch(() => [props.element.options.content, props.element.options.color, props.element.width, props.element.height], renderQrCode, { deep: true })
