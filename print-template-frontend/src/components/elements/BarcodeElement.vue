<template>
  <div
    class="element-wrapper barcode-element"
  ...
  >
    <div class="element-content" :style="contentStyle">
      <svg ref="svgRef" :width="element.width" :height="element.height"></svg>
    </div>
...
function renderBarcode() {
  ...
  try {
    JsBarcode(svgRef.value, props.element.options.content || '1234567890', {
      format: props.element.options.format || 'CODE128',
      width: 2,
      height: props.element.height - (props.element.options.showText ? 20 : 0),
      displayValue: props.element.options.showText !== false,
      margin: 0
    })
  } catch (e) {
...
watch(() => [props.element.options.content, props.element.options.format, props.element.options.showText, props.element.width, props.element.height], renderBarcode, { deep: true })
